...
data class UIBusStop(
    ...
    val name: String,
    val wifi: String,
    val lines: List<Pair<String,List<UIArrive>>>) : Parcelable, ClusterItem {
/*
...
*/
    ...
    fun toDomain(): BusStop = BusStop(node,geometry.toDomain(),name,wifi,lines.map { Pair(it.first,it.second.map {arrive-> arrive.toDomain() }) })
}
...
@Parcelize
data class UIArrive(val line: String,
                  val stop: String,
